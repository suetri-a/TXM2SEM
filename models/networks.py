...
class SRResnetGenerator(nn.Module):
    ...
    def __init__(
        ...
        Parameters:
            input_nc (int)      -- the number of channels in input images
            output_nc (int)     -- the number of channels in output images
            mult (int)          -- downsampling factor for input images
            ngf (int)           -- the number of filters in the last conv layer and residual blocks
            norm_layer          -- normalization layer
            use_dropout (bool)  -- if use dropout layers
            n_blocks (int)      -- the number of ResNet blocks
            padding_type (str)  -- the name of padding layer in conv layers: reflect | replicate | zero

        Based on SR ResNet architecture from: https://arxiv.org/pdf/1609.04802.pdf
        """
        assert(n_blocks >= 0)
        ...
        for _ in range(n_blocks):       # add ResNet blocks
            model += [SRResnetBlock(ngf, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
...
class SRResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=p, bias=use_bias), norm_layer(dim), nn.PReLU()] # nn.ReLU(True)
        if use_dropout:
            conv_block += [nn.Dropout(0.5)]
